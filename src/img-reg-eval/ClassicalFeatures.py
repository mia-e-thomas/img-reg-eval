...
class Feature(): 
    ...
    def __init__(self, config_feature):
        ...
        try:
            if config_feature['kwargs'] is None: config_feature['kwargs'] = {}
        except: 
            config_feature['kwargs'] = {}

        # Choose feature constructor based on parameters
        if config_feature['method'] == 'orb': 
            self.method = cv2.ORB_create(**config_feature['kwargs'])

        elif config_feature['method'] == 'sift': 
            self.method = cv2.SIFT_create(**config_feature['kwargs'])

        elif config_feature['method'] == 'lghd': 
            self.method = LGHD(**config_feature['kwargs'])

        else: 
            raise ValueError('Unknown alignment method: ' + config_feature['method'])





# LGHD implementation from MultiPoint Project: https://github.com/ethz-asl/multipoint/blob/main/multipoint/models/ClassicDetectors.py
